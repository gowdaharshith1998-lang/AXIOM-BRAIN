...
from axiom.providers import llm
...
def test_post_get_delete_lifecycle(api_client: TestClient) -> None:
    resp = api_client.post("/api/secrets", json={
...
def test_post_duplicate_returns_409(api_client: TestClient) -> None:
    api_client.post("/api/secrets", json={
        "provider_id": "anthropic",
        "key_name": "dup",
        "plaintext": "first",
    })
...
def test_post_test_secret_returns_verify_result(
    ...
    monkeypatch.setattr(
        "axiom.studio.vault_api.verify_secret", mock_verify
    )
...
def test_post_secret_vault_locked_returns_503(locked_client: TestClient) -> None:
    resp = locked_client.post("/api/secrets", json={
        "provider_id": "anthropic",
        "key_name": "k",
        "plaintext": "value",
    })
...
def test_rotate_secret_replaces_value(api_client: TestClient) -> None:
    ...
    resp = api_client.put("/api/secrets/anthropic/rot", json={
        "provider_id": "anthropic",
        "key_name": "rot",
        "plaintext": "new-value",
    })
...
def test_rotate_nonexistent_returns_404(api_client: TestClient) -> None:
    resp = api_client.put("/api/secrets/anthropic/nope", json={
        "provider_id": "anthropic",
        "key_name": "nope",
        "plaintext": "value",
    })
...
def test_rotate_mismatched_path_body_returns_400(api_client: TestClient) -> None:
    ...
    resp = api_client.put("/api/secrets/anthropic/mm", json={
        "provider_id": "openai",
        "key_name": "mm",
        "plaintext": "v",
    })
