...
from axiom.govern.agent_actions import synthetic_action_payload
...
def test_policy_evaluator_allows_by_default_with_zero_deny_rate() -> None:
    evaluator = SyntheticPolicyEvaluator(deny_rate=0, rng=random.Random(1))
...
def test_synthetic_action_payload_contains_demo_contract() -> None:
    payload = synthetic_action_payload(random.Random(2))
...
def test_agent_action_payload_is_async_safe() -> None:
    async def run() -> dict[str, object]:
        ...
        return synthetic_action_payload(random.Random(3))
